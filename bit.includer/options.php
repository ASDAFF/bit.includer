...
if($REQUEST_METHOD=="POST" && strlen($Update.$Apply.$RestoreDefaults)>0 && check_bitrix_sessid())
{
	...
	{
		COption::RemoveOption("bit.includer");
	}
	else
	{
		foreach($arAllOptions as $arOption)
		{
			...
			if($arOption[2][0]=="checkbox" && $val!="Y")
				$val="N";
			COption::SetOptionString("bit.includer", $name, $val, $arOption[1]);
		}
	}
...
<?$tabControl->BeginNextTab();?>
        ...
        <td>
            <select name="iblock_type" onchange="changeIblockList(this.value)">
                ...
                <?foreach ($arIBTypes as $ibtype_id => $ibtype_name):?>
                    <option value="<?= $ibtype_id?>" <?if($ibtype_id == COption::GetOptionString("bit.includer", 'iblock_type')){echo ' selected="selected"';}?>><?= $ibtype_name?></option>
                <?endforeach;?>
            </select>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <select id="iblock_id" name="iblock_id">
                <?if (COption::GetOptionString("bit.includer", 'iblock_type',0)!==0):?>
                    <?foreach ($arIB[ COption::GetOptionString("bit.includer", 'iblock_type',0) ] as $iblock_id => $iblock):?>
                        <option value="<?= $iblock_id?>"<? if($iblock_id == COption::GetOptionString("bit.includer", 'iblock_id')){echo ' selected="selected"';}?>><?= $iblock?></option>
                    <?endforeach;?>
                <?else:?>
	...
	foreach($arAllOptions as $arOption):
		...
		$val = COption::GetOptionString("bit.includer", $arOption[0], $arOption[2]);
		$type = $arOption[3];
	?>
	<tr>
